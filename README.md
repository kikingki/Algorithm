...
#### 21.1.20
  + #### [BOJ 10866](../master/10866번.py)
    정수를 저장하는 덱(Deque)을 구현한 다음, 입력으로 주어지는 프로그램을 작성하는 문제. 리스트를 이용해 덱을 구현했다. insert(), append(), pop() 메서드를 사용해 8가지 명령을 처리했다. 다양한 리스트 함수를 정확히 활용할 수 있게 됐고, 리스트를 끝에서부터 불러오고 싶으면 인덱스로 -n을 사용하면 간결히 해결할 수 있음을 알게 됐다.
