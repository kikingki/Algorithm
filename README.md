...
#### 22.6.26
  ...
  + #### [BOJ 10773번](../master/10773번.py)
    입력된 수가 0이면 가장 최근에 쓴 수를 지우고 아니면 해당 수를 쓴다고 했을 때 최종적으로 쓴 수들의 합을 구하는 문제. 가장 최근에 쓴 수를 지우려면 LIFO 구조인 stack을 사용해서 조건대로 코드를 작성하고 stack 요소들의 합을 출력하면 되는 가벼운 문제였다.

#### 22.6.27
  + #### [프로그래머스 주식가격](../master/주식가격.py)
    초 단위로 기록된 주식 가격이 담긴 배열 prices가 매개변수로 주어질 때, 현재 시간 시점의 주식 가격이 떨어지지 않고 몇 초를 버텼는지 구하는 문제. 완전 탐색으로는 1초 마다 time 값을 계산하면서 주식 가격이 떨어지는 순간의 time 값을 answer에 저장한다. 스택과 index를 이용해 각 i초 시점의 유지 시간을 계산하는 방법도 있다. 스택이 비어있지 않는 경우 끝까지 가격이 떨어지지 않은 경우이므로 따로 값을 추가해준다. 스택의 index를 해당 초 시점으로 사용하는 아이디어가 놀라웠고 지문 이해가 어려웠던 문제였다.
  + #### [프로그래머스 비밀지도](../master/비밀지도.py)
    정수 배열 arr1, arr2의 암호를 해독해 문자열로 출력하는 문제. 두 배열 모두 format 함수를 써서 10진수 배열을 2진수 2차원 배열로 변환한다. 그 후 1은 벽, 0은 공백으로 취급해 arr1[y][x]과 arr2[y][x]가 모두 0일 경우에만 문자열에 공백을 추가하고 아니면 #을 추가하여 answer을 구한다. 비트연산자를 사용하면 더 간단하게 풀 수 있다.
  + #### [프로그래머스 다트게임](../master/다트게임.py)
    "점수|보너스|[옵션]"으로 이루어진 문자열 3세트를 입력으로 받아 게임 규칙에 따라 계산한 점수 합계를 구하는 문제. 영역별 보너스 점수는 딕셔너리, 점수 계산은 스택을 이용하고 두글자인 '10'만 스택에 넣기 전에 16진수로 대체해서 넣는다. 이 후, 한 글자씩 반복을 돌면서 숫자면 스택에 push하고, 보너스 혹은 각 옵션은 규칙에 맞게 점수를 계산해준다. 반복이 끝난 후 스택의 합계가 총점이다.

#### 22.9.4
  + #### [BOJ 11650](../master/11650번.py)
    2차원 평면 위의 점 N개가 주어질 때의 좌표를  오름차순으로 정렬한 다음 출력하는 문제. 입력 값의 범위가 N(1 ≤ N ≤ 100,000)이므로 input과 readline의 시간 차가 10배 이상이다. 각 점의 좌표를 2차원 배열로 저장한 후 람다를 이용하면 한 줄로 x좌표 오름차순 정렬, x좌표가 같으면 y좌표 오름차순 정렬이 가능하다. -를 붙여주면 역순 정렬도 가능하다. 정렬 후 f-string을 이용해 출력했다.